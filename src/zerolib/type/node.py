...
class Node(Struct):
    ...
    def parents(self) -> list[tuple[Node, dict]]:
        return self.ctx.graph.parents(self)

    @cached_child_edge_property
    def children(self) -> list[tuple[Node, dict]]:
    ...
    def add_node(self) -> None:
        return self.ctx.graph.add_node(self)

    def has_child(self, node: Node) -> bool:
        return self.ctx.graph.has_child(self, node)

    def add_child(self, child: Node, **kwargs: Any) -> None:
        log.debug(f"{self!r} add_child {kwargs and f'{kwargs} ' or ''}{child!r}")
        self.ctx.graph.add_child(self, child, kwargs)
        self._clear_edge_cache(child)

    def remove_child(self, child: Node) -> None:
        log.debug(f"{self!r} remove_child {child!r}")
        self.ctx.graph.remove_child(self, child)

    def ancestors(self, filter: NodeFilterType = None) -> list[Node]:
        return self.ctx.graph.ancestors(self, filter)

    def descendants(self, filter: NodeFilterType = None) -> list[Node]:
